...
local DEFAULT_CONFIG = {
  ...
  border = "rounded",
  indent = "    ",
  data_file = string.format("%s/lazydo_tasks.json", fn.stdpath("data")), -- Corrected data_file path
...
function LazyDo:load_tasks()
  local file = io.open(self.config.data_file, "r")
  if file then
...
function LazyDo:save_tasks()
  local file = io.open(self.config.data_file, "w")
  if file then
  ...
  if is_selected and self.config.effects.highlight_current then
    ...
    for i, line in ipairs(shadow_lines) do
      ...
      table.insert(self.highlights, {
        "LazyDoGlow", #shadow_lines + i - 1, 0, #highlight
      })
    end
  end
...
vim.api.nvim_create_autocmd({ "VimLeavePre", "BufUnload" }, {
  callback = function()
...
function LazyDo:setup_keymaps()
  ...
  map('n', 'i', function() self:insert_template_task() end)
end
...
function LazyDo:create_fancy_header(width)
  local header = {
    lines = {},
    highlights = {}
  }
  ...
  for _, line in ipairs(logo) do
    ...
    table.insert(header.lines, padded_line)

    -- Add gradient highlight
    table.insert(header.highlights, {
      "LazyDoGradient", #header.lines - 1, padding, padding + #line
    })
  end

  -- Add separator
  local separator = string.rep(self.config.icons.separator, width)
  table.insert(header.lines, separator)
  table.insert(header.highlights, {
    "LazyDoSeparator", #header.lines - 1, 0, -1
  })

  -- Add stats
  local stats = string.format(
    " Total: %d | Pending: %d | Done: %d | Overdue: %d ",
    #self.tasks,
    #vim.tbl_filter(function(t) return t.status == "PENDING" end, self.tasks),
    #vim.tbl_filter(function(t) return t.status == "DONE" end, self.tasks),
    #vim.tbl_filter(function(t)
      return t.status == "PENDING" and t.due_date and t.due_date < os.date("%Y-%m-%d")
    end, self.tasks)
  )

  local stats_padding = math.floor((width - #stats) / 2)
  table.insert(header.lines, string.rep(" ", stats_padding) .. stats)
  table.insert(header.highlights, {
    "LazyDoStats", #header.lines - 1, stats_padding, stats_padding + #stats
  })
...
-- Add task filtering
function LazyDo:filter_tasks(criterion)
  vim.ui.select({
    "All",
    "Pending",
    "Done",
    ...
    elseif choice == "By Priority" then
      vim.ui.select({ "HIGH", "MEDIUM", "LOW", "NONE" }, {
        prompt = "Select priority:",
      }, function(priority)
        if priority then
      ...
      return
    elseif choice == "By Tag" then
      local all_tags = self:get_all_tags()
      vim.ui.select(all_tags, {
        prompt = "Select tag:",
      }, function(tag)
        if tag then
          filtered = vim.tbl_filter(function(task)
            return vim.tbl_contains(task.tags or {}, tag)
          end, self.tasks)
          self:show_filtered_tasks(filtered)
        end
      end)
...
-- Show filtered tasks
function LazyDo:show_filtered_tasks(filtered_tasks)
  self.filtered_tasks = filtered_tasks
  self:render()
end

function LazyDo:get_due_date_color(task)
  if task.status == "DONE" then
    return "LazyDoDueDone"
  end
...
function LazyDo:insert_template_task()
  local new_task = vim.deepcopy(template_task)
  new_task.id = tostring(os.time()) .. math.random(1000, 9999)
  new_task.created_at = os.time()
  new_task.folded = false

  table.insert(self.tasks, new_task)
  self:save_tasks()
  self:render()
  notify("Template task inserted")
end

function LazyDo:safe_operation(operation, ...)
...
function LazyDo:handle_error(err, context)
  vim.notify(
    string.format("LazyDo error in %s: %s", context, tostring(err)),
    vim.log.levels.ERROR
  )
end

function LazyDo:safe_render()
  if not is_valid_instance(self) then
    return
  end
  ...
  if not ok then
    self:handle_error(err, "render")
  end
end

function LazyDo:validate_task(task)
  ...
  for field, expected_type in pairs(required_fields) do
    if type(task[field]) ~= expected_type then
      return false, string.format("Invalid %s: expected %s", field, expected_type)
    end
  end
...
function LazyDo:add_task_safe(task_data)
  local valid, err = self:validate_task(task_data)
  if not valid then
    self:handle_error(err, "add_task")
    return
  end

  table.insert(self.tasks, task_data)
  self:save_tasks()
  self:safe_render()
end

function LazyDo:update_task_safe(task_id, updates)
  if not is_valid_instance(self) then
    return
  end
...
function LazyDo:delete_task_safe(task_id)
  if not is_valid_instance(self) then
    return
  end
...
function LazyDo:reset_state()
  self.tasks = {}
  self.selected_task_index = nil
  self.filtered_tasks = nil
  self.task_marks = {}
  self:safe_render()
end

function LazyDo:get_state()
  return {
    tasks = vim.deepcopy(self.tasks),
    selected_task_index = self.selected_task_index,
    filtered_tasks = self.filtered_tasks and vim.deepcopy(self.filtered_tasks) or nil
  }
end
-- Create content for a single task including its box and highlights
---@param task table The task to create content for
---@param width number The width of the task box
---@param is_selected boolean Whether this task is currently selected
---@return table, table Returns lines and highlights for the task
function LazyDo:create_task_content(task, width, is_selected)
  ...
  table.insert(highlights, { "LazyDoBoxBorder", #lines - 1, 0, -1 })
  ...
  table.insert(lines, title_line .. string.rep(" ", width - #title_line - 1) .. box_style.v)
  
  -- Add highlights for status and priority
  local status_hl = task.status == "DONE" and "LazyDoDone" or "LazyDoPending"
  table.insert(highlights, { status_hl, #lines - 1, #box_style.v + #padding + 1, #box_style.v + #padding + 2 })
  table.insert(highlights, { "LazyDoPriority" .. (task.priority or "NONE"), #lines - 1, #box_style.v + #padding + 3, #box_style.v + #padding + 4 })
  ...
  if task.due_date and task.due_date ~= "" then
      ...
      table.insert(lines, due_line .. string.rep(" ", width - #due_line - 1) .. box_style.v)
      table.insert(highlights, { "LazyDoDue", #lines - 1, #box_style.v + #padding, -2 })
  end

  -- Notes section if not folded
  if not task.folded and task.notes and task.notes ~= "" then
      for _, note_line in ipairs(vim.split(task.notes, "\n")) do
          ...
          table.insert(lines, formatted_line .. string.rep(" ", width - #formatted_line - 1) .. box_style.v)
          table.insert(highlights, { "LazyDoNote", #lines - 1, #box_style.v + #padding, -2 })
      end
  end
...
-- Create footer with statistics and help text
---@param width number The width of the footer
---@return table Contains lines and highlights for the footer
function LazyDo:create_footer(width)
  ...
  -- Add separator
  local separator = string.rep(self.config.icons.separator, width)
  table.insert(footer.lines, separator)
  table.insert(footer.highlights, { "LazyDoSeparator", #footer.lines - 1, 0, -1 })
  ...
  table.insert(footer.highlights, { "LazyDoHelp", #footer.lines - 1, help_padding, help_padding + #help })
