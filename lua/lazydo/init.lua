...
local function setup_highlights()
    local highlights = {
        ...
        LazyDoGradient = { fg = DEFAULT_CONFIG.colors.gradient.start, bg = DEFAULT_CONFIG.colors.gradient.end },
    ...
    if not task.folded then
        ...
        if task.subtasks and #task.subtasks > 0 then
            for _, subtask in ipairs(task.subtasks) do
                ...
                table.insert(highlights, {subtask_color, #box - 1, #box_style.v + #padding + #self.config.icons.subtask + 1, #box_style.v + #padding + #self.config.icons.subtask + 2})
                table.insert(highlights, {"LazyDoSubtaskTitle", #box - 1, #box_style.v + #padding + #self.config.icons.subtask + 3, -2})
            end
        end
